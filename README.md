...
正式分发以用户明确启用的映射为发布意图。目标文件不存在时新增；目标文件已存在时一律覆盖，不因大小或 SHA256 相同而跳过。启用“覆盖前备份”时，必须先成功备份并校验旧文件，再执行覆盖；备份失败则保护原文件、不继续覆盖。覆盖完成后继续执行大小与可选 SHA256 最终校验。该规则同时适用于单文件映射与目录映射中的同名文件。


### v0.6.75 远程常用目录
远程文件首页与分发映射的远程目录浏览器会通过 WinRM 读取当前登录用户的桌面、下载、文档、图片、音乐、视频真实路径，并与 C:/D:/E: 等磁盘一起显示。
