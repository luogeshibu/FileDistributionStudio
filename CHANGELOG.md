...
## v0.6.80
- Windows 打包安全修复：发布前移除构建链可能产生的 `_context_attributes.py` 外部源码文件。
- 新增发布硬检查：最终 EXE 目录中若仍存在任何 `.py/.pyw`，构建直接失败，不再发布可能触发 Windows 安全中心告警的包。
- 清理完成后再运行 EXE `--self-test`，确保不是以删除运行时必需文件换取“无告警”。
- PyInstaller 分析阶段显式排除 `_context_attributes`，作为第二层防护。
- 不修改 WinRM、分发、备份、目录合并覆盖或远程文件逻辑。
# v0.6.80
